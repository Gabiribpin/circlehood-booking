...
test.describe('Dashboard — Configurações', () => {
  test('carrega heading Configurações', async ({ page }) => {
    ...
    await expect(page.getByText('Configurações')).toBeVisible({ timeout: 15_000 });
  });

  test('mostra cards de Plano e Conta', async ({ page }) => {
    await page.goto(`${BASE}/settings`);
    await expect(page.getByText(/plano.*assinatura|assinatura.*plano/i)).toBeVisible({ timeout: 15_000 });
  ...
  test('botão Salvar Alterações visível', async ({ page }) => {
    await page.goto(`${BASE}/settings`);
    await expect(page.getByRole('button', { name: /salvar alterações/i })).toBeVisible({ timeout: 15_000 });
  });
});
