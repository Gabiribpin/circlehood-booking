...
describe('WhatsAppRateLimiter', () => {
  beforeEach(() => {
    // Force cleanup of all entries by calling _cleanup with future time
    // The rate limiter uses in-memory Map, so we just need fresh state
  ...
  it('allows messages below the hourly limit', () => {
    ...
    // 30th message should still be allowed
    const result = WhatsAppRateLimiter.check(phone);
    expect(result.allowed).toBe(true);
  });
